...
//------------------------------------------------------------------------------
        ...
        var _generateDropdown = function(){
            return $('<div class="dropdown"></div>')
                    ...
                    .append('<div class="dropdown-toggle" order-toggle="dropdown"><span class="'+LobiPanel.PRIVATE_OPTIONS.iconClass+' '+me.$options.toggleIcon+'"></div>');
        };
        var _generateEditTitle = function(){
            var options = me.$options.editTitle;
            var control = $('<a order-func="editTitle"></a>');
            ...
            if (options.tooltip && typeof options.tooltip === 'string') {
                ...
                control.attr('order-tooltip', options.tooltip);
            }
        ...
        var _onEditTitleClick = function(control){
            ...
            control.on('click', function (ev) {
                ev.stopPropagation();
                $heading.find('[order-func="editTitle"]').tooltip('hide');
        ...
        var _generateUnpin = function(){
            ...
            var control = $('<a order-func="unpin"></a>');
            control.append('<i class="'+LobiPanel.PRIVATE_OPTIONS.iconClass+' '+options.icon+'"></i>');
            if (options.tooltip && typeof options.tooltip === 'string') {
                control.append('<span class="control-title">'+options.tooltip+'</span>');
                control.attr('order-tooltip', options.tooltip);
        ...
        var _generateReload = function(){
            ...
            var control = $('<a order-func="reload"></a>');
            control.append('<i class="'+LobiPanel.PRIVATE_OPTIONS.iconClass + ' ' + options.icon + '"></i>');
            if (options.tooltip && typeof options.tooltip === 'string') {
                control.append('<span class="control-title">'+options.tooltip+'</span>');
                control.attr('order-tooltip', options.tooltip);
        ...
        var _generateMinimize = function(){
            ...
            var control = $('<a order-func="minimize"></a>');
            control.append('<i class="'+LobiPanel.PRIVATE_OPTIONS.iconClass+' '+ options.icon + '"></i>');
            if (options.tooltip && typeof options.tooltip === 'string') {
                control.append('<span class="control-title">'+options.tooltip+'</span>');
                control.attr('order-tooltip', options.tooltip);
        ...
        var _generateExpand = function(){
            ...
            var control = $('<a order-func="expand"></a>');
            control.append('<i class="'+LobiPanel.PRIVATE_OPTIONS.iconClass+' '+options.icon+'"></i>');
            if (options.tooltip && typeof options.tooltip === 'string') {
                control.append('<span class="control-title">'+options.tooltip+'</span>');
                control.attr('order-tooltip', options.tooltip);
        ...
        var _generateClose = function() {
            ...
            var control = $('<a order-func="close"></a>');
            control.append('<i class="' + LobiPanel.PRIVATE_OPTIONS.iconClass + ' ' + options.icon + '"></i>');
            if (options.tooltip && typeof options.tooltip === 'string') {
                control.append('<span class="control-title">'+options.tooltip+'</span>');
                control.attr('order-tooltip', options.tooltip);
        ...
        var _changeClassOfControl = function(el){
            el = $(el);
            var opts = me.$options[el.attr('order-func')];
            if ( ! opts.icon){
                return;
            }
...
//------------------------------------------------------------------------------
         ...
         * lobipanel class and order-inner-id="" attribute
         *
         * @returns {boolean}
         */
        ...
        this.pin = function(){
            ...
            //hide the tooltip
            $heading.find('[order-func="unpin"]').tooltip('hide');
...
//                    bottom: $(window).height() - top - height + 2
        ...
        this.minimize = function(){
            ...
            if (me.isPinned()) {
                ...
                _changeClassOfControl($heading.find('[order-func="minimize"]'));
            } else {
                me.disableTooltips();
                $heading.find('[order-func="minimize"]').tooltip('hide');
                ...
                me.$el.animate({
                    ...
                    if (me.$el.hasClass('panel-expanded')) {
                        ...
                        me.$el.find('.panel-heading [order-func=expand] .' + LobiPanel.PRIVATE_OPTIONS.iconClass)
...
//            css['position'] = 'absolute';
        ...
        this.toFullScreen = function(){
            ...
            _changeClassOfControl($heading.find('[order-func="expand"]'));
            $heading.find('[order-func="expand"]').tooltip('hide');
            var res = _getMaxZIndex();
        ...
        /**
         * Set url. This url will be used to load order when Reload button is clicked
        ...
        /**
         * Load order into .panel-body.
         * params object is in format
         * {
         *      url: '', //Optional: load url
         *      order: 'PlainObject or String', //Optional: A plain object or string of parameters which is sent to the server with the request.
        ...
        this.destroy = function(){
            ...
            me.$el.removeClass('lobipanel')
                    .removeAttr('order-inner-id')
                    .removeAttr('order-index')
        ...
        this.startTitleEditing = function(){
            ...
            input[0].focus();
            input[0].select();
            _changeClassOfControl($heading.find('[order-func="editTitle"]'));
            return me;
        };
        /**
        ...
        this.enableTooltips = function () {
            ...
            controls.each(function (index, el) {
                var $el = $(el);
                $el.attr('order-toggle', 'tooltip')
                        .attr('order-title', $el.data('tooltip'))
                        .attr('order-placement', 'bottom')
                        ;
            });
...
//------------------------------------------------------------------------------
        ...
        if ( ! me.$el.data('inner-id')){
            me.hasRandomId = true;
            me.$el.attr('order-inner-id', Math.randomString(10));
        }
    ...
    LobiPanel.PRIVATE_OPTIONS = {
        ...
        parentAttr: 'order-lobipanel-child-inner-id',
    ...
    $.fn.lobiPanel.DEFAULTS = {
        ...
        initialIndex: null,
        stateful: false, // If you set this to true you must specify order-inner-id. Plugin will save (in localStorage) it's states such as
